...
export async function recordEvent(typeOrObj, payload = {}) {
  try {
    ...
    if (typeOrObj && typeof typeOrObj === "object" && !Array.isArray(typeOrObj)) {
      // legacy style: recordEvent({ type, userId, courseName, at, roundId, plan, meta })
      p = typeOrObj || {};
    ...
    // ✅ plan + meta (for alerts/subscriptions segmentation)
    const planRaw =
      p.plan ??
      p.plan_name ??
      p.subscriptionPlan ??
...
async function getAlertsByPlan7d() {
  ...
  // If plan isn't provided yet, this returns empty — that's fine.
  const { rows } = await db.query(
    `
    SELECT
  ...
  // Return in a stable object shape the frontend can consume
...
async function getAvgTimeToHitMins7d() {
  ...
  // For each hit in last 7d, find the nearest preceding sent (same user + course),
  // then average the time delta in minutes.
  const { rows } = await db.query(
    `
    WITH hits AS (
