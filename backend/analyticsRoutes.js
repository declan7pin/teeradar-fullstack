...
// ✅ ADD (needed): direct Postgres queries for "user searches only" + 7d alert stats
...
async function handleSummary(req, res) {
  try {
    ...
    let s = {};
    try {
      const maybe = await getPgAnalyticsSummary();
      // ✅ CRITICAL: never allow s to be null/undefined/non-object (prevents page breaking)
      s = maybe && typeof maybe === "object" ? maybe : {};
    } catch (e) {
      console.warn("Falling back to analyticsDb summary:", e?.message || e);
      const maybe = typeof getAnalyticsSummary === "function" ? getAnalyticsSummary() : {};
      s = maybe && typeof maybe === "object" ? maybe : {};
    }

    /**
     * ✅ SEARCHES should be REAL user searches only.
    ...
    const response = {
      ...
      // ✅ IMPORTANT: searches = user searches only (not alert scraping)
      ...
      // ✅ NEW: Alerts section (7D) — multiple key names for compatibility
      alertsSent7d,
      ...
      // plan split not available without adding plan metadata to events
      alertsByPlan: null,
    };
